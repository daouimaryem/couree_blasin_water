...
# Initialize Earth Engine with service account credentials (CI/CD environment)
# or interactive authentication (local development)
def init_earth_engine():
    try:
        # Try to initialize with service account credentials from CI/CD
        if os.path.exists('gcp_key.json'):
            ...
            try:
                ...
                ee.Authenticate()
                ee.Initialize()
            print("✓ Earth Engine authenticated interactively")
...
def get_rainfall_ee(dataset_id, start_date, end_date, variable_name):
    ...
    try:
        ...
        print(f"⚠ Error fetching rainfall data from {dataset_id}: {e}")
        # Return fallback data
        dates = pd.date_range(start_date, end_date, freq='D')
        return pd.DataFrame({'date': dates, 'precip_mm': np.random.gamma(0.5, 3.5, len(dates))})
...
def main():
    ...
    print("Running water balance simulations for each building...")
    ...
    print("Flattening results for QGIS integration...")
    qgis_flat_output = []
